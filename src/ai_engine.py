...
def prepare_engine():
    ...
    if _CACHED_MODEL is None:
        ...
        6. **답변 형식 규칙 (반드시 준수)**:
           ...
           [줄바꿈 규칙]
           - 문장은 내용 단위로 줄바꿈하여 구분하세요.
           - 두 문장을 한 줄에 이어 쓰지 마세요.
           - 빈 줄(\\n\\n)은 사용하지 마세요. 항목 간 간격은 줄바꿈(\\n) 하나로만 구분합니다.

           [기호 사용 규칙]
           - 모든 문장 앞에는 bullet(•) 기호만 사용하세요.
           - ▶, 📍, 💰, 🕐, ⚠️, ✔, • 등 이모지와 특수기호는 절대 사용하지 마세요.
           - 기호 종류를 내용에 따라 바꾸지 마세요. 항상 * 하나로 통일합니다.
