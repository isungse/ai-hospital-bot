...
def prepare_engine():
    ...
    if _CACHED_MODEL is None:
        ...
        6. **답변 형식 규칙 (최우선 준수 — 이 규칙을 어기면 답변이 무효 처리됩니다)**:

           [핵심 규칙: 한 줄에 한 문장]
           - 반드시 한 줄에 한 문장만 작성하세요.
           - 각 줄은 "• "로 시작하세요. (• + 공백 1칸)
           - 줄바꿈(\\n)으로 문장을 구분하세요.
           - 절대로 한 줄 안에 "." 뒤에 또 다른 문장을 이어 쓰지 마세요.
           - 절대로 한 줄 안에 • 기호를 2개 이상 넣지 마세요.
           - ▶, 📍, 💰, 🕐, ⚠️, ✔, * 등 다른 기호는 사용 금지입니다.

           [출력 형식 예시]
           아래 형식을 그대로 따라하세요:

           • 보호자 손목 바코드 라벨은 원무과 입·퇴원 창구에서 발급받으실 수 있어요.
           • 병동 간호사실에서도 신청 가능해요.
           • 야간이나 휴일에는 응급원무과에서 도움받으실 수 있어요.

           [위반 예시 — 이렇게 쓰면 안 됩니다]
           • 손목 라벨은 원무과에서 발급받아요. • 입·퇴원 창구를 이용해 주세요.
           → 위 문장은 한 줄에 •가 2번 나왔으므로 위반입니다.
