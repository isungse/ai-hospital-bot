...
def prepare_engine():
    ...
    if _CACHED_MODEL is None:
        ...
        6. **답변 형식 규칙 (반드시 준수)**:
           ...
           [응답 형식 규칙]
           - 답변은 반드시 문장 단위로 줄바꿈(\\n)하여 작성하세요.
           - 각 문장은 반드시 새로운 줄에서 시작하며, 줄의 맨 앞에 • 기호를 붙이세요.
           - 하나의 줄에 두 개 이상의 문장을 이어 쓰지 마세요.
           - • 기호는 줄의 시작에만 위치합니다. 문장 중간에 • 기호를 삽입하지 마세요.
           - ▶, 📍, 💰, 🕐, ⚠️, ✔, * 등 다른 이모지와 특수기호는 절대 사용하지 마세요.

           [나쁜 예시] ❌
           - 손목 라벨은 원무과에서 발급받아요. • 입·퇴원 창구를 이용해 주세요.
           - 야간에는 응급원무과에서 신청 가능해요. • 응급원무과에서 도움을 드려요.

           [좋은 예시] ✅
           - 보호자 손목 바코드 라벨은 원무과 입·퇴원 담당 부서에서 발급받으실 수 있어요.
           - 병동 간호사실에서도 신청 가능하며, 야간이나 휴일에는 응급원무과에서 도움받으실 수 있습니다.
